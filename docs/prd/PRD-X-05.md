...
- **A3 · `.task` en la plantilla de View y en los ejemplos** en vez de `.onAppear { send(.load) }`; investigar `.onAppear` con `chrome: .custom`: reproducir con un XCUITest en `Examples/LoginApp` (o en AppStarter) haciendo 20 pushes; si se reproduce, buscar la causa en `ScreenContainer.customChromeStack` (`PopGestureEnabler` como `.background`) y corregir.
